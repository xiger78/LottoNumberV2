# 로또번호 (ロト番号)

> **메뉴얼:** [English](README.md) · [한국어](README_KO.md) · [日本語](README_JP.md)

일본 **로또7(Loto7)** 본숫자 패턴 분석 및 자동 생성 — React Native 앱 사용·개발 메뉴얼

> **앱 이름:** ロト番号 (일본어 표기)  
> **버전:** 1.4  
> **패키지:** `com.lotto7.generator`  
> **저장소:** https://github.com/xiger78/LottoNumber

---

## 1. 앱 개요

**ロト番号**는 일본 로또7의 과거 본숫자 **680회** 데이터를 분석하여, 통계 패턴 기반으로 번호 10조합을 자동 생성하는 React Native 앱입니다.

| 항목 | 내용 |
|------|------|
| 대상 복권 | 일본 Loto7 (1~37번 중 7개 선택) |
| 데이터 출처 | `assets/draws.json` (680회 내장) |
| 기본 표시 언어 | 일본어 (日本語) |
| 지원 언어 | 日本語 / 한국어 / English |
| 플랫폼 | React Native (Expo) — iOS & Android |

---

## 2. 화면 구성

TopBar 아래에 **LOTO 7 배너**가 항상 표시됩니다.  
하단 네비게이션으로 **5개 메뉴**를 전환합니다.

```
┌─────────────────────────────┐
│  TopBar (메뉴명)             │
├─────────────────────────────┤
│  ★ 로또 배너 (LOTO 7)        │
├─────────────────────────────┤
│                             │
│  각 메뉴 콘텐츠              │
│                             │
├─────────────────────────────┤
│로또│당첨│조회│이력│설정      │
└─────────────────────────────┘
```

---

## 3. 메뉴별 기능 설명

### 3.1 ロト番号 (로또번호)

엑셀 본숫자 데이터를 분석하여 번호를 자동 생성하는 메인 화면입니다.

**주요 기능**

- **680회** 과거 추첨 데이터 패턴 분석 (빈도, 口 분포, 홀짝, 합계)
- **월별 패턴** 선택 (1~12月) — 해당 월 출현 빈도 반영
- **번호 10조합 생성** — 가중치 랜덤 + 패턴 필터 적용
- **저장된 당첨숫자 반영** — 최근 당첨 번호 가중치 감소, 미출현 번호 가중치 증가
- **공식 당첨 발표 사이트** 링크 → [みずほ銀行 Loto7](https://www.mizuhobank.co.jp/takarakuji/check/loto/loto7/index.html)

![ロト番号 화면 (한국어 UI)](docs/images/ko/screen_lotto.png)

---

### 3.2 当選数字 (당첨숫자)

발표된 당첨 본숫자를 등록·수정·삭제하고, **최신 추첨 데이터를 자동으로 갱신**하는 화면입니다.  
등록된 번호는 **ロト番号** 메뉴의 생성 알고리즘에 자동 반영됩니다.

**자동 갱신 (v1.4)**

- **앱 시작 시** — 내장 `draws.json`(680회)과 공식 사이트 최신 회차를 당첨숫자 DB에 자동 등록
- **당첨숫자 탭 진입 시** — 미등록 회차를 다시 확인·갱신
- **회차 내림차순** 표시 (第680回 → 第679回 …), 상단 **총 N회** 표시
- 갱신 중 상단 **진행 표시줄** 표시

> 설정의 **본숫자 자동등록**을 누르지 않아도 위 화면에서 최신 데이터가 표시됩니다.

**수동 입력**

- **추가 (+)** — 회차, 추첨일, 본숫자 7개 입력
- **수정 / 삭제** — 확인 후 삭제
- Room DB에 영구 저장

**입력 형식 예시**

```
회차: 第677回
추첨일: 2026年6月13日
본숫자: 01 05 12 17 23 28 34
```

![당첨숫자 (한국어 UI)](docs/images/ko/screen_winning.png)

---

### 3.3 当選照会 (당첨번호 조회)

내장 추첨 데이터(**680회**)의 과거 당첨 본숫자를 조회하는 화면입니다.

**주요 기능**

- **회차·날짜 검색**
- **최신순** 정렬
- **10건씩** 페이지 이동 (이전 / 다음)

![당첨번호 조회 (한국어 UI)](docs/images/ko/screen_lookup.png)

---

### 3.4 生成履歴 (생성이력)

자동 생성한 로또 번호의 이력을 확인하는 화면입니다.

**표시 형식 (한국어)**

`2026년06월11일14시30분:01 02 03 04 05 06 07`

**기능**

- **일시 내림차순** 정렬 (최신순)
- **10건씩** 페이지 단위 표시

![생성이력 (한국어 UI)](docs/images/ko/screen_history.png)

---

### 3.5 設定 (설정)

표시 언어 및 당첨번호 자동 등록 기능을 제공합니다.

**표시 언어**

1. **日本語** (기본)
2. **한국어**
3. **English**

**자동 등록 버튼**

| 버튼 | 기능 |
|------|------|
| **본숫자 자동등록** | 내장 데이터(680회) **미등록분** 등록 후, 최신 내장 회차 이후 번호는 공식 사이트에서 추가 조회·등록 |
| **공식 사이트에서 가져오기** | みずほ銀行 당첨 발표 페이지에서 **최신 번호**를 가져와 등록 (네트워크 필요) |

**본숫자 자동등록 동작**

- 설정 화면 **상단 배너**에 진행 상태 및 결과 표시 (예: `등록 4건 / 건너뜀 676건`)
- 1단계: `draws.json` 내장 데이터에서 미등록 회차 일괄 등록
- 2단계: 내장 데이터 최신 회차(680회) 이후는 공식 사이트에서 추가 확인

> 공식 사이트는 봇 차단으로 실패할 수 있습니다. 이 경우에도 내장 데이터에 포함된 회차는 **본숫자 자동등록**으로 등록됩니다.

![설정 (한국어 UI)](docs/images/ko/screen_settings.png)

---

## 4. 번호 생성 알고리즘

```
1. 내장 680회 + 저장된 당첨숫자 → 번호별 가중치 계산
2. 월별 패턴 가중치 추가
3. 가중치 랜덤으로 7개 번호 선택
4. 패턴 필터 적용
   - 口 분포 (1~7 / 8~14 / 15~21 / 22~28 / 29~37)
   - 홀수 개수 (3~4개가 가장 많음)
   - 합계 범위 (평균 ± 1.8σ)
   - 저장된 당첨 조합과 5개 이상 겹치면 제외
5. 10조합 생성 → 생성이력에 자동 저장
```

> ※ 과거 패턴 기반 **참고용**이며 당첨을 보장하지 않습니다.

---

## 5. 개발 환경

### 5.1 필수 도구

| 도구 | 버전 |
|------|------|
| **Node.js** | 18+ |
| **Expo** | `npx expo` |
| **iOS** | Xcode (macOS) |
| **Android** | Android Studio / SDK |

### 5.2 실행

```bash
npm install
npx expo start
```

`a` — Android 에뮬레이터, `i` — iOS 시뮬레이터, QR 코드 — Expo Go

### 5.3 빌드

```bash
npm install -g eas-cli
eas build --platform android
eas build --platform ios
```

---

## 6. 기술 스택

| 분류 | 기술 |
|------|------|
| 프레임워크 | React Native + Expo |
| 언어 | TypeScript |
| 네비게이션 | React Navigation |
| 저장소 | AsyncStorage |
| 데이터 | `assets/draws.json` |

### 프로젝트 구조

```
LottoNumber/
├── App.tsx
├── assets/draws.json
├── src/engine/          # 패턴 분석·번호 생성
├── src/data/            # 저장소·공식 사이트 fetch
├── src/screens/         # 5개 탭 화면
├── src/i18n/            # 3개국어
└── docs/images/
```

---

## 8. 변경 이력

### v1.4

- **당첨숫자** 메뉴 앱 시작·탭 진입 시 자동 동기화 (680회, 최신순)
- 서명된 **정식 APK** 빌드 (`assembleRelease`)

### v1.3.1

- **본숫자 자동등록 UX 개선** — 설정 화면 상단 결과 배너, “이미 등록됨” 메시지 명확화, 등록 속도 개선
- 내장 데이터 등록 / 공식 사이트 확인 중 진행 상태 표시

### v1.3

- 내장 추첨 데이터 **680회** 확장 (677~680회 추가)
- **본숫자 자동등록** 시 내장 최신 회차 이후 번호를 공식 사이트에서 추가 조회

### v1.2

- **당첨번호 조회** 화면 추가 (676회, 검색, 페이지)
- 설정: **본숫자 자동등록**, **공식 사이트에서 가져오기**
- 하단 메뉴 5개 구성

### v1.1

- 상단 **LOTO 7 배너** 추가
- 메뉴얼 및 화면 캡처 문서

### v1.0

- 4개 메뉴, 676회 패턴 분석, 3개국어, GitHub 등록

---

## 9. 면책 조항

본 앱은 과거 추첨 데이터의 통계적 패턴을 참고하여 번호를 생성하는 **오락·참고용** 도구입니다.  
로또 당첨을 보장하거나 확률을 높인다는 과학적 근거는 없습니다.
